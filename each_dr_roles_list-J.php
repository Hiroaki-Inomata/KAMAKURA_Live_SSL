...
$sql = "SELECT * FROM `session_tbls2018` INNER JOIN `role_tbls` ON  `session_tbls2018`.`sessionNo` = `role_tbls`.`sessionNo` AND ";
$sql .= "`session_tbls2018`.`class` = `role_tbls`.`class` WHERE `role_tbls`.`dr_tbl_id` = :dr_tbl_id;";
...
<body>
	<h3 class="text-center text-danger">
		<?= $row_dr['english_sirname']." ".$row_dr['english_firstname']; ?>さんのお仕事リスト</h3>
  ...
  <?= mb_substr($latest, 0, 10) ?>
	...
	<div class="row col-md-11 text-center">
		...
		<div class="col-sm-12">
			<p><a class="btn btn-danger" id="return" href="../index2018.php">トップページに戻ります</a>
			</p>
		</div>
	</div>
